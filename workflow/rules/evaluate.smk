def _get_evaluation_regions(wildcards):
    if wildcards.stratification == "genome":
        return f"data/truth/{wildcards.sample}.{wildcards.reference}.bed"
    else:
        return f"data/truth/{wildcards.sample}.{wildcards.reference}.{wildcards.stratification}.bed"
...
rule vcfeval:
    input:
        ...
        evaluation_regions=_get_evaluation_regions,
    ...
    output:
        directory("results/eval/{sample}.{library}.{depth}x.{reference}.{mapper}.{caller}.{filter}.{match}.{stratification}.vcfeval")
    log:
        "logs/eval/{sample}.{library}.{depth}x.{reference}.{mapper}.{caller}.{filter}.{match}.{stratification}.vcfeval.log"
    params:
