...
rule download_giab:
    ...
    wildcard_constraints:
        ...
        reference='|'.join([re.escape(s) for s in REFERENCES])
    params:
...
giab_genome_stratifications = {
    "GRCh38": {
        "alldifficult": "ftp://ftp-trace.ncbi.nlm.nih.gov//giab/ftp/release/genome-stratifications/v2.0/GRCh38/union/GRCh38_alldifficultregions.bed.gz",
        "alllowmapandsegdup": "ftp://ftp-trace.ncbi.nlm.nih.gov//giab/ftp/release/genome-stratifications/v2.0/GRCh38/union/GRCh38_alllowmapandsegdupregions.bed.gz",
        "notinalldifficult": "ftp://ftp-trace.ncbi.nlm.nih.gov//giab/ftp/release/genome-stratifications/v2.0/GRCh38/union/GRCh38_notinalldifficultregions.bed.gz",
        "notinalllowmapandsegdup": "ftp://ftp-trace.ncbi.nlm.nih.gov//giab/ftp/release/genome-stratifications/v2.0/GRCh38/union/GRCh38_notinalllowmapandsegdupregions.bed.gz",
        "AllTandemRepeatsandHomopolymers": "https://ftp-trace.ncbi.nlm.nih.gov/ReferenceSamples/giab/release/genome-stratifications/v2.0/GRCh38/LowComplexity/GRCh38_AllTandemRepeatsandHomopolymers_slop5.bed.gz",
        "notinAllTandemRepeatsandHomopolymers": "https://ftp-trace.ncbi.nlm.nih.gov/ReferenceSamples/giab/release/genome-stratifications/v2.0/GRCh38/LowComplexity/GRCh38_notinAllTandemRepeatsandHomopolymers_slop5.bed.gz"
    }
}
giab_genome_stratifications["hs38DH"] = giab_genome_stratifications["GRCh38"]

rule download_genome_stratification:
    output:
        "data/reference/stratifications/{reference}.{stratification}.bed.gz"
    params:
        url = lambda wildcards: giab_genome_stratifications[wildcards.reference][wildcards.stratification]
    shell:
        "curl -o {output} {params.url}"

localrules: download_genome_stratification

rule make_stratified_truth_bed:
    input:
        full = "data/truth/{sample}.{reference}.bed",
        stratification = "data/reference/stratifications/{reference}.{stratification}.bed.gz"
    output:
        "data/truth/{sample}.{reference}.{stratification}.bed"
    wildcard_constraints:
        reference='|'.join([re.escape(s) for s in REFERENCES])
    conda:
        "../envs/bedtools.yaml"
    shell:
        "bedtools intersect -a {input.full} -b {input.stratification} > {output}"

localrules: make_stratified_truth_bed
